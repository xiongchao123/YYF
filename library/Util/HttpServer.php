...
class HttpServer implements HttpRequestInterface{
     ...
     */
    public function set_proxy($host,$port = 0,$user = '',$pass = ''){
    ...
    private function _http_request($url,$post_data,$referer=''){
        ...
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, $this->timeout); //timeout on connect
        curl_setopt($ch, CURLOPT_TIMEOUT, $this->timeout);
        //强制使用IPV4协议解析域名，否则在支持IPV6的环境下请求会异常慢
        @curl_setopt($ch, CURLOPT_IPRESOLVE, CURL_IPRESOLVE_V4);
        if ($this->request_method === 'POST' || $post_data){
